 ...
 */
public class QLearningAgentTrial {
...
//		System.out.println("! " + recCounter + " records are clear.");
	 ...
	 */
	QLearningAgentTrial agentClone() {

		// 学習エージェントを生成する
		QLearningAgentTrial clonedAgent = new QLearningAgentTrial();
	...
	double learn(ProjectState preState, ProjectManagementAction action, double reward, ProjectState postState) {
		...
		int iPrePrgR = discretizeProgressRate(dPrePrgR);

		// 変化前状態量 SPI の値を離散化する
		double dPreSpi = preState.getSPI();
		int iPreSpi = discretizeSpi(dPreSpi);

		// 変化前状態量 CPI の値を離散化する
		double dPreCpi = preState.getCPI();
		int iPreCpi = discretizeCpi(dPreCpi);

		// 変化前状態量 averageApplyingPressure の値を離散化する
		double dPreAvgAppPrs = preState.getAverageAP();
		int iPreAvgAppPrs = discretizeAverageApplyingPressure(dPreAvgAppPrs);

		// 変化前状態量 averageIncreasingEfforts の値を離散化する
		double dPreAvgIncEff = preState.getAverageIE();
		int iPreAvgIncEff = discretizeAverageIncreasingEfforts(dPreAvgIncEff);
